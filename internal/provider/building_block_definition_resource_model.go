...
type buildingBlockDefinitionSpec struct {
	client.MeshBuildingBlockDefinitionSpecAdapter[generic.Value[clientTypes.String], generic.Value[supportedPlatformRef]]
}
...
func (model buildingBlockDefinitionSpec) ToClientDto(diags *diag.Diagnostics) client.MeshBuildingBlockDefinitionSpec {
	dto := client.MeshBuildingBlockDefinitionSpec{
		MeshBuildingBlockDefinitionSpecBase: model.MeshBuildingBlockDefinitionSpecBase,
		// Set computed fields
		Symbol: model.Symbol.GetPtr(diags),
	}
	for _, supportedPlatform := range model.SupportedPlatforms {
		dto.SupportedPlatforms = append(dto.SupportedPlatforms, supportedPlatform.Get(diags).Name)
	}
	return dto
}

func (model *buildingBlockDefinitionSpec) SetFromClientDto(dto client.MeshBuildingBlockDefinitionSpec, diags *diag.Diagnostics) {
	if slices.Compare(model.NotificationSubscribers, dto.NotificationSubscribers) != 0 {
		diags.AddAttributeError(
			path.Root("spec").AtName("notification_subscribers"),
			"Inconsistent response received",
			fmt.Sprintf(
				"The backend returned the list %s, but the config required %s. "+
					"This means that at least one provided is not accepted by the backend.",
				dto.NotificationSubscribers, model.NotificationSubscribers),
		)
		return
	}
	model.MeshBuildingBlockDefinitionSpecBase = dto.MeshBuildingBlockDefinitionSpecBase
	model.Symbol.SetOptional(dto.Symbol, diags)
	if len(model.SupportedPlatforms) == 0 {
		model.SupportedPlatforms = nil
	}
	if len(model.NotificationSubscribers) == 0 {
		model.NotificationSubscribers = nil
	}
}
...
type buildingBlockDefinitionVersionSpec struct {
	...
	client.MeshBuildingBlockDefinitionVersionSpecAdapter[
		*buildingBlockDefinitionVersionInput,
		generic.Value[client.BuildingBlockRunnerRef],
		generic.Value[buildingBlockDefinitionRef],
		secret.Secret,
...
func (model *buildingBlockDefinition) SetVersionRefsFromClientDto(
	...
	for _, versionDto := range versionDtos {
		versionRefs = append(versionRefs, buildingBlockDefinitionVersionRef{
			...
			ContentHash: versionContentHash(versionDto.Spec, diags),
		})
	}
	if diags.HasError() {
		return
...
func (model buildingBlockDefinitionVersionSpec) ToClientDto(buildingBlockDefinitionUuid string, secretSupplier secret.PlaintextSupplier, diags *diag.Diagnostics) (dto client.MeshBuildingBlockDefinitionVersionSpec) {
	...
	if model.RunnerRef.IsNull() || model.RunnerRef.IsUnknown() {
		implementationType := model.Implementation.InferTypeFromNonNilField()
		...
		dto.RunnerRef = model.RunnerRef.Get(diags)
	}

	if model.Draft {
		dto.State = ptr.To(client.MeshBuildingBlockDefinitionVersionState(client.MeshBuildingBlockDefinitionVersionStateDraft))
	} else {
		dto.State = ptr.To(client.MeshBuildingBlockDefinitionVersionState(client.MeshBuildingBlockDefinitionVersionStateReleased))
	}
...
func (model *buildingBlockDefinitionVersionSpec) SetFromClientDto(dto client.MeshBuildingBlockDefinitionVersionSpec, diags *diag.Diagnostics) {
	...
	// Note that there Drafts might be reviewed before release, and while in Review, can still be edited!
	model.Draft = *dto.State == client.MeshBuildingBlockDefinitionVersionStateDraft.Unwrap()

	model.VersionNumber.SetRequired(dto.VersionNumber, diags)
	model.State.SetRequired(dto.State, diags)
	model.RunnerRef.SetRequired(&dto.RunnerRef, diags)

	if len(model.DependencyDefinitionUUIDs) == 0 {
		model.DependencyDefinitionUUIDs = nil
	}
	...
	if dto.Implementation.Type == "manual" && dto.Implementation.Manual == nil {
		...
		model.Implementation.Manual = &client.MeshBuildingBlockDefinitionManualImplementation{}
	} else if terraformImplDto := dto.Implementation.Terraform; terraformImplDto != nil {
	...
	for inputKey, inputDto := range dto.Inputs {
		...
		if len(input.SelectableValues) == 0 {
			input.SelectableValues = nil
		}
