...
func TestAccLandingZoneDataSource(t *testing.T) {
	landingZoneConfig, landingZoneAddr := testconfig.BuildLandingZoneAndWorkspaceConfig(t)

	dsAddress := testconfig.Traversal{"data.meshstack_landingzone", "example"}
	dsConfig := testconfig.DataSource{Name: "landingzone"}.Config(t)
	dsConfig = dsConfig.WithFirstBlock(t,
		testconfig.Traverse(t, "metadata", "name")(testconfig.SetRawExpr(landingZoneAddr.Format("%s.metadata.name"))))
	config := dsConfig.Join(landingZoneConfig)
