...
func TestAccWorkspaceGroupBinding(t *testing.T) {
	...
	bindingConfig := testconfig.Resource{Name: "workspace_group_binding"}.Config(t)
	...
	bindingConfig = bindingConfig.WithFirstBlock(t,
		...
		testconfig.Traverse(t, "target_ref", "name")(testconfig.SetRawExpr(workspaceAddr.Format("%s.metadata.name"))))

	config := bindingConfig.Join(workspaceConfig)
