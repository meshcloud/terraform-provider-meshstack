...
func TestAccLandingZone(t *testing.T) {
	config, lzAddr := testconfig.BuildLandingZoneAndWorkspaceConfig(t)
	resourceAddress := lzAddr.String()

	updateConfig := config.WithFirstBlock(t,
		testconfig.Traverse(t, "spec", "display_name")(testconfig.SetString("Updated Landing Zone")))
