...
import (
	...
	clientTypes "github.com/meshcloud/terraform-provider-meshstack/client/types/ptr"
...
func (r *buildingBlockDefinitionResource) Create(ctx context.Context, req resource.CreateRequest, resp *resource.CreateResponse) {
	...
	createRequest := client.MeshBuildingBlockDefinition{
		...
		Spec:     plan.Spec.ToClientDto(&resp.Diagnostics),
	}
	if resp.Diagnostics.HasError() {
		return
	}
	...
	plan.Spec.SetFromClientDto(createdDto.Spec, &resp.Diagnostics)
	if resp.Diagnostics.HasError() {
		return
	}
	...
	createdVersionDto, err := r.buildingBlockDefinitionVersionClient.Update(ctx, versionUuid, plan.Metadata.OwnedByWorkspace, versionSpecDto)
	if err != nil {
	...
	plan.Ref = generic.ToObject(ctx, buildingBlockDefinitionRef{
		Kind: createdDto.Kind,
		Uuid: bbdUuid,
	}, plan.Ref.AttributeTypes, &resp.Diagnostics)
...
func (r *buildingBlockDefinitionResource) ModifyPlan(ctx context.Context, req resource.ModifyPlanRequest, resp *resource.ModifyPlanResponse) {
	if !req.State.Raw.IsKnown() || req.State.Raw.IsNull() {
		// do nothing in case of create for now (one could potentially populate the Versions output though
		return
	...
	var plan, state buildingBlockDefinition
	resp.Diagnostics.Append(req.Plan.Get(ctx, &plan)...)
	resp.Diagnostics.Append(req.State.Get(ctx, &state)...)
	if resp.Diagnostics.HasError() {
		return
	...
	versionSpecDto := plan.VersionSpec.ToClientDto(bbdUuid, secret.NewPlaintextSupplierForModifyPlan(ctx, req), &resp.Diagnostics)
	if resp.Diagnostics.HasError() {
		return
	}
	...
	case !state.VersionSpec.Draft && plan.VersionSpec.Draft:
		...
		latestVersionRef := generic.FromObject[buildingBlockDefinitionVersionRef](ctx, state.VersionLatest, &resp.Diagnostics)
		latestVersionRef.Number++
		latestVersionRef.State = client.MeshBuildingBlockDefinitionVersionStateDraft.Unwrap()
		latestVersionRef.ContentHash = versionContentHash(versionSpecDto, &resp.Diagnostics)
		plan.VersionLatest = generic.ToObject(ctx, latestVersionRef, state.VersionLatest.AttributeTypes, &resp.Diagnostics)
		generic.SetAttribute(ctx, &plan.VersionLatest, "uuid", types.StringUnknown(), &resp.Diagnostics)
		if resp.Diagnostics.HasError() {
			return
		}
		generic.AppendElement(ctx, &plan.Versions, plan.VersionLatest, &resp.Diagnostics)
		// don't forget to show the plan changes of those "dependent" computed values
		plan.VersionSpec.State.SetRequired(&latestVersionRef.State, &resp.Diagnostics)
		plan.VersionSpec.VersionNumber.SetRequired(&latestVersionRef.Number, &resp.Diagnostics)
		if resp.Diagnostics.HasError() {
			return
		}
	case !state.VersionSpec.Draft:
		// state (and plan) are in draft=false (aka released), so nothing should change (mark them all as "known from state")
		plan.VersionLatestRelease = state.VersionLatestRelease
		plan.Versions = state.Versions
		plan.VersionLatest = state.VersionLatest
	default:
		// State is in draft=true, and plan might have draft=false,
		// so update the content hash of version latest (and last element of versions)
		// and set VersionLatestRelease to new released version if required
		plan.Versions = state.Versions
		latestVersionRef := generic.FromObject[buildingBlockDefinitionVersionRef](ctx, state.VersionLatest, &resp.Diagnostics)
		latestVersionRef.State = *versionSpecDto.State
		latestVersionRef.ContentHash = versionContentHash(versionSpecDto, &resp.Diagnostics)
		...
		if latestVersionRef.State == client.MeshBuildingBlockDefinitionVersionStateReleased.Unwrap() {
			plan.VersionLatestRelease = plan.VersionLatest
			plan.VersionSpec.State.SetRequired(client.MeshBuildingBlockDefinitionVersionStateReleased.Ptr(), &resp.Diagnostics)
		} else {
			// keep as is, no release happened
			plan.VersionLatestRelease = state.VersionLatestRelease
		}

		generic.SetLastElement(ctx, &plan.Versions, plan.VersionLatest, &resp.Diagnostics)
		if resp.Diagnostics.HasError() {
			return
		}
	}
...
func (r *buildingBlockDefinitionResource) Update(ctx context.Context, req resource.UpdateRequest, resp *resource.UpdateResponse) {
	var plan buildingBlockDefinition
	resp.Diagnostics.Append(req.Plan.Get(ctx, &plan)...)
	var stateVersionSpecDraft bool
	resp.Diagnostics.Append(req.State.GetAttribute(ctx, path.Root("version_spec").AtName("draft"), &stateVersionSpecDraft)...)
	var bbdLatestVersion buildingBlockDefinitionVersionRef
	resp.Diagnostics.Append(req.State.GetAttribute(ctx, path.Root("version_latest"), &bbdLatestVersion)...)
	bbdUuid := plan.Metadata.Uuid.Get(&resp.Diagnostics)
	if resp.Diagnostics.HasError() {
		return
	}
	...
	plan.Spec.SetFromClientDto(updatedDto.Spec, &resp.Diagnostics)
	if resp.Diagnostics.HasError() {
		return
	}
	...
	switch {
	case !stateVersionSpecDraft && plan.VersionSpec.Draft:
		// changing draft=false->true means creating a new draft version from the existing one with increased version number
		versionSpecDto.VersionNumber = clientTypes.To(bbdLatestVersion.Number + 1)
	...
	case !stateVersionSpecDraft:
		...
		if versionSpecDtoContentHash == bbdLatestVersion.ContentHash {
			...
			resp.Diagnostics.AddError("Error updating version_spec", fmt.Sprintf(
				...
				bbdLatestVersion.ContentHash, versionSpecDtoContentHash,
			))
			return
		}
	default:
		// State is in draft=true, so we update the version_spec (even if content hashes are equal)
		latestVersionUuid := bbdLatestVersion.Uuid
