...
// building block through a sequence of steps. Several scenarios mix steps that run against the
// in-memory mock with steps that need the real backend; the latter carry SkipFunc: skipInMock, and
// where a both-modes step has assertions that only hold against the real backend, the
// ConfigStateChecks are gated on IsMockClientTest().
...
func TestAccBuildingBlock(t *testing.T) {
	...
	// 01_workspace_lifecycle walks a workspace-targeted building block through its full life: create,
	// import, in-place updates (display_name, an input value, content_hash), and finally a parent
	// change that must force a replace.
	//
	// What to watch:
	//   - The empty-plan step (step 3) and the input-value/content_hash draft steps (steps 5-7) run in
	//     both modes: editing inputs in the same workspace is an in-place Update and a content_hash bump
	//     re-runs the block, all without a Replace. The mock materializes the same null input rows the
	//     real backend echoes, so the no-drift check holds in both modes.
	//   - The parent-change step (step 8) runs mock-only: it asserts the provider-side plan decision
	//     that adding a parent forces a destroy+recreate (RequiresReplaceIf). The real backend rejects
	//     the synthetic parent-BB UUID, and that plan logic is identical in both modes, so it is
	//     skipped in acceptance.
	...
	// evaluate.
	//
	// What to watch: the v1 resource has no wait_for_completion, so its run is still PENDING after
	// create; the move is an Update and updating a non-terminal block hits the backend's
	// completed-state guard (409 conflict). The PreConfig awaits SUCCEEDED first — acceptance-only,
	// since the mock has no such run.
	...
	// 05_operator_inputs walks platform-operator inputs (PLATFORM_OPERATOR_MANUAL_INPUT, here the `size`
	// input) through their full lifecycle on a cross-workspace building block: the BBD is owned by
	// workspace A and the block lives in consumer workspace B, managed by the default (admin) provider
	// (setting an operator input on create and creating a block in another workspace are admin-only).
	// The block is:
	//   - created WITHOUT `size`, so on a real backend it parks in WAITING_FOR_OPERATOR_INPUT (surfaced
	//     as a warning, not an error);
	//   - resumed by a later PUT that supplies `size` — the provider must poll THROUGH the transient
	//     WAITING to the resulting SUCCEEDED run (TestAwaitRunPollsThroughWaiting is the unit-level guard);
	//   - updated on a consumer input, leaving the operator input in place;
	//   - upgraded cross-workspace to a v2 definition that ADDS a defaulted operator input, which the
	//     backend applies on upgrade so the block reaches SUCCEEDED instead of re-parking in WAITING.
	//
	// Steps 1-4 run in BOTH modes (the mock has no operator-input WAITING state, so the create
	// short-circuits to a terminal status — the WAITING assertion is gated on the real backend). The
	// re-draft/re-release + upgrade steps need the real backend and carry SkipFunc: skipInMock.
	//
	// A MANAGED_BUILDINGBLOCK_SAVE key is still minted here (workspace A): in mock mode this proves the
	// provider accepts MANAGED_BUILDINGBLOCK_SAVE in the api_key permission set. A MANAGED key may NOT
	// create a block cross-workspace, and its cross-workspace operator-input/upgrade authorization is
	// covered on the backend by MeshBuildingBlockManagedSaveScenarios; 07_non_updateable_rejected is the
	// companion check for a plain workspace-scoped key.
  ...
  environment = { value = jsonencode("dev") }
		...
		ApplyAndTest(t, resource.TestCase{
			Steps: []resource.TestStep{
				...
				{
					// Acceptance-only: re-draft the BBD → v2 draft (adds the defaulted operator input);
				...
				{
					// Acceptance-only: release v2 and upgrade the cross-workspace block to it. v2 adds a
	...
	// 06_sensitive_inputs_and_upgrade is the sensitive-input scenario. One shared BBD declares a
	// STRING (api_key), a CODE (script) and a STATIC (static_secret) sensitive input, and the BB is
	// created, upgraded across a BBD re-draft/re-release, and finally rotated.
	//
	// What to watch:
	//   - Mock vs. acceptance: only create (step 1) and rotation (last step) run against the mock —
	//     the upgrade chain (import, re-draft, re-release+upgrade, empty-plan) needs the real backend
	//     and carries SkipFunc: skipInMock. The mock hashes any sensitive plaintext (STRING and CODE
	//     alike), so the api_key and script hashes are asserted in both modes; only the STATIC
	//     static_secret hash (which the mock does not resolve) is gated on !IsMockClientTest().
	//   - lastRunUuid is captured both at create and at the acceptance-only upgrade step, so the final
	//     rotation step compares against whichever ran last and still proves a rerun in both modes
	//     (rotation is invisible to the rerun predicate and must be detected via secret_version).
	//   - The version ref points at version_latest_release.uuid (resolvable in both modes), so the BB
	//     stays pinned to the released version while a new draft exists; on the upgrade PUT the
	//     sensitive hash sentinel must preserve the original secret rather than corrupt it.
	t.Run("06_sensitive_inputs_and_upgrade", func(t *testing.T) {
		...
		// (releases v2). Acceptance-only — the mock skips the upgrade steps that use these.
		...
		// Rotate api_key (new secret_value + bumped secret_version) on the post-upgrade config. In
		// mock the upgrade steps are skipped, so this applies straight after create — still a rotation.
		...
		ApplyAndTest(t, resource.TestCase{
			Steps: []resource.TestStep{
				...
				{
					// Step 2 (acceptance-only): import with verify. Besides content_hash and the
				...
				{
					// Step 3 (acceptance-only): re-draft the BBD → v2 draft. version_latest_release
				...
				{
					// Step 4 (acceptance-only): release BBD v2 + upgrade the BB to v2 in one apply. The
				...
				{
					// Step 5 (acceptance-only): post-upgrade plan must be empty — no spurious rerun and
	...
	// 08_validation_rejects collects the plan-time and create-time rejection checks into one scenario
	// over the workspace BBWorkspace config: a target_ref whose kind and identifier disagree, a
	// malformed version uuid, and a STATIC BBD input wrongly assigned as a customer input.
	//
	// What to watch: the target_ref kind/identifier mismatch checks (steps 1-2) are provider-side
	// validators and run in both modes; the malformed-uuid 400 (step 3) and the STATIC-input
	// rejection (steps 4-5) need the real backend and carry SkipFunc: skipInMock. The ExpectError
	// steps establish no state, so ordering them before the valid create in step 4 is safe; in mock
	// only steps 1-2 run.
  ...
  value = jsonencode("eu-central-1")
		...
		ApplyAndTest(t, resource.TestCase{
			Steps: []resource.TestStep{
				...
				{
					// Acceptance-only: a malformed version uuid must surface as a clear backend 400
					// (not a 500). \s+ tolerates Terraform's word-wrapping of the diagnostic detail.
					SkipFunc:    skipInMock,
				...
				{
					// Acceptance-only: create a valid BB first, then (next step) attempt the invalid
					// STATIC-input assignment as an Update.
					SkipFunc: skipInMock,
					Config:   config.String(),
					ConfigPlanChecks: resource.ConfigPlanChecks{
						PreApply: []plancheck.PlanCheck{
				...
				{
					// Acceptance-only: assigning the STATIC input as a customer input must be rejected.
					SkipFunc:    skipInMock,
	...
	// 09_purge_on_delete sets purge_on_delete = true so teardown deletes the building block via the
	// DELETE /{uuid}/purge path. Runs in both modes: it exercises the create + flag wiring and that the
	// purge teardown succeeds. The purge itself soft-deletes (the block briefly stays readable with a
	// DELETED lifecycle), but teardown also destroys the building block definition, and definition deletion
	// hard-removes soft-deleted blocks referencing it — so by the time CheckDestroy runs the block is gone
	// and the GET 404s. This proves the purge teardown path completed, not that a deprovision run was
	// skipped (the no-op acceptance runner cannot show that). The mock ignores the purge flag and hard-
	// removes on delete too.
	...
	// 12_moved_from_v2_with_secret guards the v2→v3 migration of a block that carries a sensitive
	// USER_INPUT. After the move the real secret cannot ride through state (secret_value is
	// write-only), but it must NOT be lost: re-declaring the input with a DISTINCT placeholder
	// secret_value and NO secret_version preserves the existing secret — moveFromV2 seeds the
	// sensitive block so the input stays managed (not dropped to all_inputs by SetFromClientDto), the
	// post-move refresh recovers the real secret_hash from the backend, and the apply echoes that hash
	// (secret.ValueToConverter: unchanged version → sends {Hash}). So the backend keeps the original
	// value and the all_inputs hash stays equal to the v2 block's. The distinct placeholder makes
	// corruption detectable: if the provider sent the placeholder plaintext, the hash would change.
	// Runs in both modes — the mock hashes sensitive plaintext and keeps it on a matching hash echo
	// (backendSecretBehavior), just like the backend. The BBD points at the local bare repo so the
	// acceptance runs clone fast (like 06) instead of starving the shared tf-runner.
...
// non-empty metadata.uuid, the display name, the always-present "name" input, and a non-empty status
// (plus a non-empty latest_run_uuid in mock mode). Callers append scenario-specific input checks via
// extra — the workspace and tenant examples both pass bbv3SizeEnvInputChecks.
func bbv3StateChecks(buildingBlockAddr testconfig.Traversal, displayName string, extra ...statecheck.StateCheck) []statecheck.StateCheck {
	...
	if IsMockClientTest() {
		checks = append(checks, statecheck.ExpectKnownValue(buildingBlockAddr.String(), tfjsonpath.New("status").AtMapKey("latest_run_uuid"), xknownvalue.NotEmptyString()))
	}
	return checks
}
