...
// terraformTestdataRepoURL returns a file:// URL to the committed bare git repo under
// testdata/tf-building-block (a single-commit no-op OpenTofu module). The tf-block-runner clones it
// to run terraform offline. The path must be absolute for a file:// URL; tests run from the package
// dir, so testdata/ is relative to it.
func terraformTestdataRepoURL(t *testing.T) string {
	t.Helper()
	abs, err := filepath.Abs(filepath.Join("testdata", "tf-building-block"))
	if err != nil {
		t.Fatalf("resolve testdata repo path: %v", err)
	}
	return "file://" + abs
}
...
func TestAccBuildingBlock(t *testing.T) {
	...
	// 03_workspace_moved_from_v2 guards the v2→v3 migration: a Terraform `moved` block from
  ...
  environment = { value = jsonencode("dev") }
	...
	t.Run("06_sensitive_inputs_and_upgrade", func(t *testing.T) {
		...
		bbdV1Released := exampleResource.TestSupportConfig(t, "_bbd").WithFirstBlock(
			...
			testconfig.OwnedByWorkspace(workspaceAddr),
			// Point the terraform implementation at the committed local bare repo (file://) so the
			// real tf-block-runner clones and runs OpenTofu offline. In mock mode this value is
			// unused. The static example URL in the .tf is only a docs placeholder.
