...
func TestAccTenantsV4DataSource(t *testing.T) {
	...
	dataSourceConfig := testconfig.DataSource{Name: "tenants"}.Config(t)
	dataSourceConfig = dataSourceConfig.WithFirstBlock(t,
		testconfig.Traverse(t, "workspace")(testconfig.SetRawExpr(tenantAddr.Format("%s.metadata.owned_by_workspace"))),
		testconfig.Traverse(t, "project")(testconfig.SetRawExpr(tenantAddr.Format("%s.metadata.owned_by_project"))))
