...
func TestAccPlatformDataSource(t *testing.T) {
	...
	config := testconfig.DataSource{Name: "platform"}.Config(t).
		Join(PlatformResourceConfigForTest(t, &resourceAddress, nil))
	config = config.WithFirstBlock(t,
		testconfig.Traverse(t, "metadata", "uuid")(testconfig.SetRawExpr(resourceAddress.Format("%s.metadata.uuid"))))
