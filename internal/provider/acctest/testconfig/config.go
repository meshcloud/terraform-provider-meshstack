...
func (c Config) WithFirstBlock(modifiers ...ExpressionConsumer) Config {
	c.t.Helper()
	result := clone(c)
	...
	for _, modifier := range modifiers {
		modifier(c.t, Block{c.t, blocks[0]})
	}
	return result
}

// Descend returns a modifier-builder that descends into the given attribute path steps
...
//	)
func Descend(steps ...string) func(modifiers ...ExpressionConsumer) ExpressionConsumer {
	return func(modifiers ...ExpressionConsumer) ExpressionConsumer {
...
// WalkAttributes returns a modifier-builder that visits every attribute reachable from root,
// including nested object attributes, and applies the provided modifiers to each attribute expression.
// Recursion stops at leaf values where attributes() returns nil (non-object values).
func WalkAttributes() func(modifiers ...ExpressionConsumer) ExpressionConsumer {
	return func(modifiers ...ExpressionConsumer) ExpressionConsumer {
		return func(t *testing.T, root Expression) {
			t.Helper()
