...
// pass terraformRepoUrl (a file:// URL to the committed bare repo) so the real tf-block-runner can
// clone and run OpenTofu offline in acceptance mode. In mock mode the URL is unused. workspaceAddr
...
func BBTenant(t *testing.T, terraformRepoUrl string) (config Config, buildingBlockAddr Traversal, workspaceAddr Traversal) {
	...
	buildingBlockDefinitionConfig := Resource{Name: "building_block", Suffix: "_02_tenant"}.TestSupportConfig(t, "").WithFirstBlock(
		...
		// Point the terraform implementation at the committed local bare repo (file://) so the real
		// tf-block-runner clones and runs OpenTofu offline. The static example URL in the .tf is a
		// docs placeholder; in mock mode this value is unused.
