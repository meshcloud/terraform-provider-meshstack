...
// withBody parses the attribute's tokens as an object body and calls the action.
// The action reports if the body was modified. If so, the internal state is updated.
// If the tokens don't form an object ({...}), the action is never called.
func (f fakeBlock) withBody(action func(body *hclwrite.Body, names quotedNameMap) (modified bool)) {
