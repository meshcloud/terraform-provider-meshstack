...
func TestDescendSetString(t *testing.T) {
	c := newConfig(t, []byte(`resource "test" "ex" {
  metadata = {
    name = "old-name"
    tags = "keep-me"
...
func TestDescendTreeNesting(t *testing.T) {
	c := newConfig(t, []byte(`resource "test" "ex" {
  spec = {
...
func TestDescendQuotedKeys(t *testing.T) {
	c := newConfig(t, []byte(`resource "test" "ex" {
  inputs = {
    normal = {
      type = "STRING"
...
func TestDescendSetRawExpr(t *testing.T) {
	c := newConfig(t, []byte(`resource "test" "ex" {
  metadata = {
...
func TestDescendRenameKey(t *testing.T) {
	c := newConfig(t, []byte(`resource "test" "ex" {
  spec = {
    old_name = "value"
  }
...
func TestDescendSetValue(t *testing.T) {
	c := newConfig(t, []byte(`resource "test" "ex" {
  spec = {
    draft = true
  }
...
func TestDescendDeepNesting(t *testing.T) {
	c := newConfig(t, []byte(`resource "test" "ex" {
  version_spec = {
    implementation = {
      gitlab_pipeline = {
...
func TestDescendQuotedKeysPreservedAtParentLevel(t *testing.T) {
	c := newConfig(t, []byte(`resource "test" "ex" {
  version_spec = {
    inputs = {
      normal = {
...
// into a nested tree structure and invokes the consumer on every leaf attribute.
...
func TestDescendUpsertsNewAttribute(t *testing.T) {
	c := newConfig(t, []byte(`resource "test" "ex" {
  spec = {
    name = "test"
  }
