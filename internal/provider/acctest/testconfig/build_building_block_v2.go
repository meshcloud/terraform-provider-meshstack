...
func BuildBBv2WorkspaceConfig(t *testing.T) (config Config, bbAddr Traversal) {
	t.Helper()
	workspaceConfig, workspaceAddr := BuildWorkspaceConfig(t)
	var bbdAddr Traversal
	bbdConfig := Resource{Name: "building_block_v2", Suffix: "_01_workspace"}.TestSupportConfig(t, "").WithFirstBlock(t,
		ExtractIdentifier(&bbdAddr),
		OwnedByWorkspace(t, workspaceAddr),
	)
	return Resource{Name: "building_block_v2", Suffix: "_01_workspace"}.Config(t).WithFirstBlock(t,
...
func BuildBBv2TenantConfig(t *testing.T) (config Config, bbAddr Traversal) {
	t.Helper()
	workspaceConfig, workspaceAddr := BuildWorkspaceConfig(t)
	...
	var bbdAddr Traversal
	bbdConfig := Resource{Name: "building_block_v2", Suffix: "_02_tenant"}.TestSupportConfig(t, "").WithFirstBlock(t,
		ExtractIdentifier(&bbdAddr),
		OwnedByWorkspace(t, workspaceAddr),
		Traverse(t, "spec", "supported_platforms")(SetRawExpr(fmt.Sprintf("[{name = %s}]", platformTypeAddr.Join("metadata", "name")))),
	)

	return Resource{Name: "building_block_v2", Suffix: "_02_tenant"}.Config(t).WithFirstBlock(t,
		ExtractIdentifier(&bbAddr),
		Traverse(t, "spec", "building_block_definition_version_ref")(SetRawExpr(bbdAddr.Join("version_latest").String())),
