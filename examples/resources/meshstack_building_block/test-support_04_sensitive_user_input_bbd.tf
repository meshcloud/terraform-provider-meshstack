resource "meshstack_building_block_definition" "sensitive_user_input" {
  ...
  version_spec = {
    ...
    inputs = {
      ...
      # CODE-typed sensitive USER_INPUT. The mock hashes it like a STRING, but the real backend
      # processes CODE-type sensitive inputs distinctly — so its hash is asserted acceptance-only.
      script = {
