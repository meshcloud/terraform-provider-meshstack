...
subcategory: ""
description: |-
  Manage a workspace or tenant building block using the v3-preview API.
...
- `wait_for_completion` (Boolean) Whether to wait for the building block to reach a terminal state (SUCCEEDED or FAILED) before completing create/update/delete operations.
...
- `inputs` (Attributes Map) User inputs. Set either `value` (plain string or `jsonencode(...)`) or `sensitive` for each input key. (see [below for nested schema](#nestedatt--spec--inputs))
- `inputs_platform_operator` (Attributes Map) Platform operator manual inputs. Set either `value` or `sensitive` for each input key. (see [below for nested schema](#nestedatt--spec--inputs_platform_operator))
...
- `inputs_static` (Map of String) Static or system-determined inputs from the definition version.
...
- `latest_run` (Attributes) Latest building block run. (see [below for nested schema](#nestedatt--status--latest_run))
