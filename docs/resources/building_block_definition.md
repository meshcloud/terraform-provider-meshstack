...
- `state` (String) State of the current version.
...
- `sensitive` (Attributes) Sensitive input values, mutually exclusive with non-sensitive `argument` and `default_value` attributes. You can provide an empty attribute `sensitive = {}` to mark this input sensitive without providing `argument` or `default_value`. Sensitive is only supported for `argument_type` of `USER_INPUT`, `PLATFORM_OPERATOR_MANUAL_INPUT`, `STATIC`. (see [below for nested schema](#nestedatt--version_spec--inputs--sensitive))
