...
- `updateable_by_consumer` (Boolean) Whether the input value can be updated by consumers without admin or platform operator permissions. For assignment type `PAYMENT_METHOD`, `false` makes the Payment Method picked when ordering the building block final.
