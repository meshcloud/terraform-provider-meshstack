...
- `supported_platforms` (Attributes List) List of platform identifiers that this building block supports. Required and must be non-empty if target_type is `TENANT_LEVEL` (see [below for nested schema](#nestedatt--spec--supported_platforms))
...
- `runner_ref` (Attributes) Reference to the runner to run the implementation. If not provided, the pre-defined shared runner is used suitable for the given `implementation` choice (see [below for nested schema](#nestedatt--version_spec--runner_ref))
