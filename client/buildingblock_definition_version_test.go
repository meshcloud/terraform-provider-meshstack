...
func TestMeshBuildingBlockDefinitionInput_UnmarshalJSON(t *testing.T) {
	tests := []struct {
		...
		{"sensitive", true, types.SecretOrAny{}, types.SecretOrAny{X: types.Secret{Hash: types.PtrTo("some-hash")}}, assert.NoError},
		...
		t.Run(tt.name, func(t *testing.T) {
			...
			if tt.wantErr(t, json.Unmarshal(jsonFile, &target)) {
				expected := MeshBuildingBlockDefinitionInput{
					...
					MeshBuildingBlockDefinitionInputAdapter: MeshBuildingBlockDefinitionInputAdapter[types.SecretOrAny]{
