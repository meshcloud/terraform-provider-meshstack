...
FEATURES:
- `meshstack_building_block_definition`: a `version_spec.inputs` entry can now carry a `condition`, a [CEL](https://cel.dev) expression over the other inputs of the same version, for example `condition = "input.cloud_provider == \"aws\""`. The input is only asked for while the condition holds. While it does not, meshPanel hides it and meshStack sends no value for it, so the implementation falls back to the default declared in its own code, exactly as for `is_optional`. A condition is only supported on a `USER_INPUT` or `PLATFORM_OPERATOR_MANUAL_INPUT` input and never on the `manual` implementation, which the provider checks at plan time. What the condition may read is checked by meshStack when the version is saved: a `USER_INPUT` may read other `USER_INPUT` inputs, a `PLATFORM_OPERATOR_MANUAL_INPUT` may read both, a referenced input must not be sensitive or a `LIST`, its key must contain no `-`, and conditions must not read each other in a loop. A condition is part of the content hash, so adding or changing one on a released version creates a new version; a definition without conditions keeps its hash. This needs meshStack 2026.37.0 or later; an older meshStack does not know the field and rejects the apply.
