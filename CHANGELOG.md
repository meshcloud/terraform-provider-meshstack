...
FEATURES:
- `meshstack_tenant` (resource and data source) and `meshstack_tenants` now model tenant quotas as maps keyed by quota key whose value is an object carrying the quota `value`. Requested quotas are set at creation via the new `spec.requested_quotas` map (e.g. `{ "limits.cpu" = { value = 4 } }`), and the tenant's effective quotas are read back from the new computed `status.applied_quotas` map (e.g. `{ "limits.cpu" = { value = 4 } }`). Wrapping each value in an object (rather than a bare number) matches the meshStack API and lets per-quota fields be added later without a breaking change to the map shape. These effective quotas are the ones meshStack actually applied, which can differ from `spec.requested_quotas` once the landing zone's default quotas are merged in or a platform operator adjusts them; when they differ, the provider emits a warning noting that the requested quotas were not fully applied and that a pending platform-operator approval may be the reason. The old `spec.quotas` set of `{key, value}` objects is **deprecated** but still accepted for backward compatibility; provide either `requested_quotas` or `quotas`, not both with conflicting values (the backend rejects a mismatch, or a `quotas` list carrying duplicate keys, with HTTP 400). `spec.requested_quotas` remains create-only — the meshTenant API has no quota update endpoint, so changing it on an existing tenant is still rejected at plan time; use it only to request quotas at creation. It is also a create-time input the API does not return on read, so the data sources expose it as typically null; read effective values from `status.applied_quotas`. meshStack now enforces each quota's `[minValue, maxValue]` bounds (for every caller) and the auto-approval threshold (for non-admin API keys) at creation, rejecting out-of-range or above-threshold requests with a descriptive HTTP 400 that the provider surfaces. Requires the backend carrying these changes (meshStack 2026.30.0). The deprecated `meshstack_tenant_v4` resource and data source now also expose `status.applied_quotas` (a map of `{ value }` objects) read back from the API instead of echoing the requested `spec.quotas`.
