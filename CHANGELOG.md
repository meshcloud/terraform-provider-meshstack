...
FEATURES:
- `meshstack_tenant` (resource and data source) and `meshstack_tenants` now expose the tenant's effective quotas as a computed `status.quotas` set of `{key, value}` objects, read back from the meshTenant v4 API. These are the quotas meshStack actually applied, which can differ from the requested `spec.quotas` once the landing zone's default quotas are merged in or a platform operator adjusts them. `spec.quotas` remains create-only — the meshTenant API has no quota update endpoint, so changing it on an existing tenant is still rejected at plan time; use it only to request quotas at creation. meshStack now enforces each quota's `[minValue, maxValue]` bounds (for every caller) and the auto-approval threshold (for non-admin API keys) at creation, rejecting out-of-range or above-threshold requests with a descriptive HTTP 400 that the provider surfaces. Requires the backend carrying these changes (meshStack 2026.30.0). The deprecated `meshstack_tenant_v4` resource and data source now also source `status.quotas` from the API instead of echoing the requested `spec.quotas`.
