...
FEATURES:
- `meshstack_tenant` (resource and data source) and `meshstack_tenants` now model tenant quotas as `key -> value` maps. Requested quotas are set at creation via the new `spec.requested_quotas` map (e.g. `{ "limits.cpu" = 4 }`), and the tenant's effective quotas are read back from the new computed `status.applied_quotas` map. These effective quotas are the ones meshStack actually applied, which can differ from `spec.requested_quotas` once the landing zone's default quotas are merged in or a platform operator adjusts them. The old `spec.quotas` set of `{key, value}` objects is **deprecated** but still accepted for backward compatibility; provide either `requested_quotas` or `quotas`, not both with conflicting values (the backend rejects a mismatch with HTTP 400). `spec.requested_quotas` remains create-only — the meshTenant API has no quota update endpoint, so changing it on an existing tenant is still rejected at plan time; use it only to request quotas at creation. meshStack now enforces each quota's `[minValue, maxValue]` bounds (for every caller) and the auto-approval threshold (for non-admin API keys) at creation, rejecting out-of-range or above-threshold requests with a descriptive HTTP 400 that the provider surfaces. Requires the backend carrying these changes (meshStack 2026.30.0). The deprecated `meshstack_tenant_v4` resource and data source now also expose `status.applied_quotas` (a `key -> value` map) read back from the API instead of echoing the requested `spec.quotas`.
