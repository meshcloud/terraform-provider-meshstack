...
- A profile is picked by matching its endpoint when none is named and no API key was supplied, and the provider says which one it chose in a log record — run with `TF_LOG=WARN` to see it.
